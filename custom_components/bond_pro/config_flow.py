"""Config flow for the Bond Pro integration."""
...
async def async_get_token(hass: HomeAssistant, host: str) -> str | None:
    """Try to fetch the token from the bond device (unlock window)."""
    bond = Bond(
        host,
        "",
...
class BondProConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Bond Pro."""
    ...
    async def _async_try_automatic_configure(self) -> None:
        ...
        online longer than the allowed setup period, and we will
    ...
    async def async_step_reauth(
        ...
        """Handle reauth when the token stops working."""
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            ...
            try:
                bond_id, _ = await _validate_input(self.hass, data)
            except InputValidationError as error:
                errors["base"] = error.base
            else:
                await self.async_set_unique_id(bond_id)
    ...
    async def async_step_reconfigure(
        ...
        """Allow changing the host (and token) without re-adding."""
        entry = self._get_reconfigure_entry()
        errors: dict[str, str] = {}
        if user_input is not None:
            try:
                bond_id, _ = await _validate_input(self.hass, user_input)
            except InputValidationError as error:
                errors["base"] = error.base
            else:
                await self.async_set_unique_id(bond_id)
